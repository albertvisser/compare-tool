"""unittests for ./src/json5_comp.py
"""
import types
import pytest
from src import json5_comp as testee


def test_compare_json5data(monkeypatch, capsys):
    """unittest for json5_comp.compare_json5data
    """
    def mock_read(arg):
        "stub"
    ...
    counter = 0
    assert testee.compare_json5data('oldfile', 'newfile') == [('1left', 'left'),
                                                             ('2left', 'left'),
                                                             ('1right', 'left'),
                                                             ('2right', 'right'),
                                                             ('xxx', 'both')]
...
def test_refresh_json5compare(monkeypatch, capsys):
    """unittest for json5_comp.refresh_json5compare
    """
    ...
    testee.refresh_json5compare(comparer)
    assert capsys.readouterr().out == (
...
def test_prepare_values():
    """unittest for json5_comp.prepare_values
    """
    data = []
...
def test_readjson(monkeypatch, capsys, tmp_path):
    """unittest for json5_comp.readjson
    """
    def mock_load(arg):
        print(f'called json5.load with arg {arg}')
        return 'data'
    def mock_read(arg):
    ...
    assert capsys.readouterr().out == (
            f"called json5.load with arg <_io.TextIOWrapper name='{filename}' mode='r'"
            " encoding='UTF-8'>\n"
            "called read_dict with arg data\n")


def test_read_dict(monkeypatch, capsys):
    """unittest for json5_comp.read_dict
    """
    def mock_read(arg):
...
def test_read_list(monkeypatch, capsys):
    """unittest for json5_comp.read_list
    """
    def mock_read(arg):
        print(f'called read_list with arg {arg}')
        return 'q', 'r'
...
def test_gen_next():
    """unittest for json5_comp.gen_next
    """
